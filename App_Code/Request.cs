...
public class Request: Database
{
    ...
    public List<User> getUsers(String Role)
    {
        ...
        Query = "SELECT [user].id, [user].name, [user].email, [user].contact, [user].logon, [user].confirmed " +
    ...
    public User getUser(String Logon)
    {
        User _User = new User();
        Query = "SELECT [user].id, [user].name, [user].email, [user].contact, [user].logon, [user].confirmed " +
                "FROM [user] WHERE [User].logon = '" + Logon + "'";
        try
        {
            Data.SqlCommand cm = new Data.SqlCommand();
            cm.CommandText = Query;
            ...
            if (rd.HasRows)
            {
                ...
                _User.Department = _User.getDepartment(_User.Id);
                _User.Role = _User.getRoles(_User.Id);
                _User.Confirmed = rd.GetBoolean(5);
            }
            rd.Close();
        }
        catch (Exception)
    ...
    {
        Query = "INSERT INTO [User] (name, email, logon, contact, departmentid, confirmed) VALUES ('" + _User.Name + "','" + _User.EMail + "','" + _User.Logon + "','" + _User.Phone + "'," + _User.Department.Id + "," + _User.Confirmed + ")";
        try
        {
            Data.SqlCommand cm = new Data.SqlCommand();
            cm.CommandText = Query;
    ...
    public void ModUser(User _User)
    {
        Query = "UPDATE [User] SET name = '" + _User.Name + "', email = '" + _User.EMail + "', logon = '" + _User.Logon + "', contact = '" + _User.Phone + "', departmentid = " + _User.Department.Id + ", confirmed = " + _User.Confirmed + " WHERE [User].Id = " + _User.Id;
        try
        {
            Data.SqlCommand cm = new Data.SqlCommand();
            cm.CommandText = Query;
    ...
    public bool isUser(String Logon)
    {
        bool _isUser = false;
        Query = "SELECT [user].id, [user].name, [user].email, [user].contact, [user].logon, [user].confirmed " +
                "FROM [user] WHERE [User].logon = '" + Logon + "'";
    ...
    public void addNarrative(Narrative _Narrative)
    {
        Query = "INSERT INTO Narrative (Name, FileName, Description) VALUES ('" + _Narrative.Name + "','" + _Narrative.FileName + "','" + _Narrative.Description + "')";
        try
        {
        Data.SqlCommand cm = new Data.SqlCommand();
        cm.CommandText = Query;
    ...
    public void modNarrative(Narrative _Narrative)
    {
        Query = "UPDATE Narrative SET Name = '" + _Narrative.Name + "', FileName = '" + _Narrative.FileName + "', Description = '" + _Narrative.Description + "' WHERE id = " + _Narrative.Id;
        try
        {
            Data.SqlCommand cm = new Data.SqlCommand();
            cm.CommandText = Query;
    ...
    public List<Document> getDocuments(int RequestId)
    {
        ...
        try
        {
            ...
            while (rd.Read())
            {
                ...
                _Document.User = getUser(rd.GetInt32(5));
    ...
    public List<Comment> getComments(int RequestId)
    {
        ...
        try
        {
            ...
            while (rd.Read())
            {
                ...
                _Comment.Id = rd.GetInt32(0);
                _Comment.User = getUser(rd.GetInt32(1));
                _Comment.Status = getStatusById(rd.GetInt32(2));
    ...
    public List<Workflow> getWorkflow(int RequestId)
    {
        ...
        try
        {
            ...
            while (rd.Read())
            {
                ...
                _Workflow.Id = rd.GetInt32(0);
                _Workflow.User = getUser(rd.GetInt32(2));
                _Workflow.Role = _Workflow.getRole(rd.GetInt32(1));
